        ...
        for j in range(src_shape[1]):
            ...
            if propagation_enabled == False:
                if odd_iteration:
                    ...
                    if (0 < i - 1 < src_shape[0]):
                        if (0 < i + new_f[i-1, j, 0] < src_shape[0] and 0 < j + new_f[i-1, j, 1] < src_shape[1]):
                    ...
                    if (0 < j - 1 < src_shape[1]):
                        if (0 < i + new_f[i, j -1, 0] < src_shape[0] and 0 < j + new_f[i, j - 1, 1] < src_shape[1]):
            ...
            if random_enabled == False:
                ...
                while temp >= 1:
                    ...
                    u_f = new_f[i, j] + np.multiply(temp, R)
                    if (0 < u_f[0] + i < src_shape[0] and 0 < u_f[1] + j < src_shape[1]):
                        src_temp = np.ndarray.flatten(source_patches)
                        dest_temp = np.ndarray.flatten(target_patches)
                        dist = np.abs(src_temp - dest_temp)
                        abs_dist = dist.dot(dist)
                        if (abs_dist < temp_best):
                            new_f[i, j] = u_f
                            best_D[i, j] = dist
...
def reconstruct_source_from_target(target, f):
    ...
    """
    shape = f.shape[1]
    origin_coordinate_matrix = make_coordinates_matrix(target.shape)
    target_mapping = origin_coordinate_matrix + f
    final_mapping = target.copy() * 0
    for i in range(final_mapping.shape[0]):
        for j in range(final_mapping.shape[1]):
            final_mapping[i, j] = target[target_mapping[i, j, 0], target_mapping[i, j, 1]]
    rec_source = final_mapping.reshape(-1, shape, 3)
